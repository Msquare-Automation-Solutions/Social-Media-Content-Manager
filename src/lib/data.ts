...
// ── Review queue tree ────────────────────────────────────────────────────────
// The pending-approval queue as a Platform → Category → item tree. Pure
// `buildReviewTree` (unit-tested) does the grouping; `getReviewQueue` queries.
...
export type ReviewQueue = { total: number; groups: ReviewGroup[] };
...
export function buildReviewTree(
  ...
  const leafOf = (a: ReviewAsset): ReviewLeaf => ({
    id: a.id,
    title: a.title,
    type: a.type,
    source: a.source,
  ...
  // categoryKey for a type (VIDEO_SCRIPT folds into the Videos view).
  const viewFor = (type: string): LibraryViewKey | null =>
    LIBRARY_VIEWS.find((v) => (v.types as readonly string[]).includes(type))?.key ?? null;

  // Bucket assets by platform id (or "unassigned"), preserving channel order.
  const byGroup = new Map<string, ReviewAsset[]>();
  const push = (gid: string, a: ReviewAsset) => {
  ...
  const categoriesFor = (list: ReviewAsset[]): ReviewCategory[] =>
    LIBRARY_VIEWS.map((v) => {
      const items = list.filter((a) => viewFor(a.type) === v.key);
      return { key: v.key, label: v.label, count: items.length, assets: items.map(leafOf) };
    }).filter((c) => c.count > 0);

  const groups: ReviewGroup[] = [];
  // Platforms in their workspace order, then the Unassigned bucket last.
...
export async function getReviewQueue(workspaceId: string): Promise<ReviewQueue> {
  ...
  const assets: ReviewAsset[] = rows.map((a) => ({
    id: a.id,
    title: a.title,
    type: a.type,
    source: a.source,
